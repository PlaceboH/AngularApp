...
@Component({
  selector: 'app-add-szkolenie',
  templateUrl: './add-szkolenie.component.html'
})
export class AddSzkolenieComponent implements OnInit {
  form: FormGroup;
  loading = false;
  submitted = false;
  user : User;
  ...
  onSubmit() {
    ...
    this.loading = true;
    let name : string = this.accountService.userValue.username;
    this.accountService.registerSzkolenie(this.form.value, name)
        .pipe(first())
        .subscribe(
            data => {
